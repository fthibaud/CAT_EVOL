...
type pln = (float * Xy.t) array (* [|...(time in seconds, coords in Nm)...|] *)
...
let create_pln speed ds t0 xys =
  ...
  Array.mapi (fun i xyi ->
    if i <> 0 then (
      ...
      t := !t +. Xy.norm (Xy.sub xyi xys.(i - 1)) *. dt);
    (!t, xyi)) xys
...
let create_route pln =
  let rec get t i =
    if i = Array.length pln then []
      ...
      Xy.bary pln.(i - 1) pln.(i) t::get (t +. delta) i
    else get t (i + 1) in
...
let get_vector acft =
  ...
  let i = min (Array.length acft.pln - 2) acft.leg in
  Xy.bary acft.pln.(i) acft.pln.(i + 1) (t +. vector)
...
let nav speed xys =
  ...
  let da = alpha *. Xy.radians in
  let epsilon = da /. 2. in
  let radius = speed /. hour *. std_turn /. Xy.pi /. 2. in
  let min_d = radius *. max_a in
  let rec get hdg = function
    | xy1::xy2::tl when xy1 = xy2 -> get hdg (xy1::tl)
    | xy1::xy2::tl ->
      let v = Xy.sub xy2 xy1 in
      let d = Xy.norm v in
      let dir = Xy.angle v in
      let keep_xy2 = match tl with
	| [xy] when Xy.norm (Xy.sub xy1 xy) < d +. 1. *. radius -> false
	| xy::_ -> 
	  epsilon < abs_float (Xy.sub_angle (Xy.angle (Xy.sub xy xy2)) dir)
	| _ -> true in
      if keep_xy2 then (
	let turn = Xy.sub_angle dir hdg in
	if max_a +. epsilon < abs_float turn then (
	  let h = Xy.sub_angle hdg (if turn < 0. then max_a else -. max_a) in
	  let xy = Xy.add xy1 (Xy.mul min_d (cos h, sin h)) in
	  let d2 = Xy.norm (Xy.sub xy xy2) in
	  xy1::get h (xy::if radius < d2 || tl = [] then xy2::tl else tl))
	else (
	  let a = 
	    if tl = [] then turn else (da *. float (round (turn /. da))) in
	  let h = Xy.sub_angle hdg (-.a) in
	  xy1::get h (Xy.add xy1 (Xy.mul (max d min_d) (cos h, sin h))::tl)))
      else get hdg (xy1::tl)
    | l -> l in
  get (Xy.angle (Xy.sub (List.nth xys 1) (List.hd xys))) xys
...
let dev acft t_dev xy_dev =
  ...
  let xyd = Xy.bary acft.pln.(!leg) acft.pln.(!leg + 1) t_dev in
...
let delay acft =
  ...
  let d_min = Xy.norm (Xy.sub (snd acft.pln.(l - 1)) (snd acft.pln.(0))) in
  let d = ref 0. in
  for i = 0 to l - 2 do 
    d := !d +. Xy.norm (Xy.sub (snd acft.pln.(i + 1)) (snd acft.pln.(i)));
  done;
